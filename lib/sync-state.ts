 ...
 *
 * `conflict` is preserved for TICKET-001 revision-safe sync and takes priority
 * over offline/retry phases when set by the app.
 */
...
export function syncBadgeLabel({ mode, phase }: SyncBadgeInput): string {
  ...
  switch (phase) {
    ...
    case "conflict":
      return "Cloud workspace - newer teammate edit available";
    case "demo":
...
export function syncStampLabel(phase: CloudSyncPhase): string {
  switch (phase) {
    ...
    case "conflict":
      return "Cloud rune needs latest";
    case "demo":
