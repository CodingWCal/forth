...
describe("resolveCloudSyncPhase", () => {
  ...
  it("prefers conflict over offline or retry", () => {
    expect(
      resolveCloudSyncPhase({
        mode: "cloud",
        online: false,
  ...
  it("prefers offline over syncing or retry when not in conflict", () => {
    expect(
      resolveCloudSyncPhase({
        mode: "cloud",
        online: false,
...
describe("sync copy", () => {
  it("pairs fantasy badge labels with literal save meaning", () => {
    ...
    expect(syncBadgeLabel({ mode: "cloud", phase: "conflict" })).toContain("teammate");
